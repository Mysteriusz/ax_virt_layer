/*
 	Quick tables interface for i64 architecture interfaces
*/

#if !defined(MTE_I64_QTABLES_INT)
#define MTE_I64_QTABLES_INT

#include <ax_type.h>

/*
 	For MTE_I64_IMMD_INT interface.
*/

// Act as default array for [struct immd_tables_root] members.
//
// 3 bit legacy byte to immediate table (for _i64_immd_check)
...
//
// 3 bit legacy byte to modrm table (for _i64_modrm_check)
...
/*
 	For MTE_I64_INSTR_INT interface.
*/
extern u8 LEG_PREF_LOOKUP[0xff]; // (for _i64_get_legacy)
extern u8 MOD_DLEN_LOOKUP[4]; // 2 bit modrm.mod field to displacement (for _i64_disp_len)
...
*/
_inline_avert void i64_load_qtables(
	void
);

#endif // !defined(MTE_I64_QTABLES_INT)
